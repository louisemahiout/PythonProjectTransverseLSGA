# === FONCTION MENU ET CHOIX DE NIVEAU ===
...
def run_game():
    ...
    new_w, new_h = 64, 64
    x, y = 100, screen_height - 20 - new_w
    velocity = 5
    jump_velocity = -15
    gravity = 0.8
    y_vel = 0
    ...
    walk = [pygame.transform.scale(pygame.image.load(f'Hero/Run ({i}).png').convert_alpha(), (new_w, new_h)) for i in range(1, 15)]
    idle = [pygame.transform.scale(pygame.image.load(f'Hero/Idle ({i}).png').convert_alpha(), (new_w, new_h)) for i in range(1, 15)]
    jump_anim = [pygame.transform.scale(pygame.image.load(f'Hero/Jump ({i}).png').convert_alpha(), (new_w, new_h)) for i in range(1, 17)]
    cur_frame = 0
    idle_frame = 0
    idle_counter = 0
    jump_frame = 0
    ...
    # === ENNEMIS SUR PLATEFORMES ET SOL ===
    bots = [
        pygame.Rect(terrain[1].x + 30, terrain[1].y - 30, 40, 30),
        pygame.Rect(terrain[2].x + 30, terrain[2].y - 30, 40, 30),
        pygame.Rect(terrain[3].x + 30, terrain[3].y - 30, 40, 30),
        pygame.Rect(terrain[4].x + 30, terrain[4].y - 30, 40, 30),
        pygame.Rect(terrain[5].x + 30, terrain[5].y - 30, 40, 30),
        pygame.Rect(terrain[6].x + 30, terrain[6].y - 30, 40, 30),
        pygame.Rect(terrain[7].x + 30, terrain[7].y - 30, 40, 30),
        pygame.Rect(600, screen_height - 50, 40, 30),
        pygame.Rect(1000, screen_height - 50, 40, 30),
        pygame.Rect(1500, screen_height - 50, 40, 30),
    ]
    bot_speed = 3
    bot_y_vel = [0] * len(bots)  # Vitesse verticale des bots
    lives = 3
    invincible = False
    inv_time = 0
    INV_DURATION = 2000
    ...
    while running:
        ...
        # Appliquer la gravité aux bots
        for i, b in enumerate(bots):
            bot_y_vel[i] += gravity
            b.y += bot_y_vel[i]
            hitbox = pygame.Rect(b.x, b.y, b.width, b.height)

            for rect in terrain:
                r = rect.move(-scroll_x, 0)
                if hitbox.colliderect(r):
                    if bot_y_vel[i] > 0:  # Le bot tombe
                        b.y = r.top - b.height
                        bot_y_vel[i] = 0  # Stoppe la chute
                    elif bot_y_vel[i] < 0:  # Le bot monte
                        bot_y_vel[i] = 0  # Stoppe la montée

        # Déplacement horizontal des bots
        for b in bots:
            player_world_x = x + scroll_x
            if abs(b.x - player_world_x) <= 200:
                b.x += bot_speed if b.x < player_world_x else -bot_speed
